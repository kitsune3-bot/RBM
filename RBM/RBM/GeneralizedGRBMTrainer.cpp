...
GeneralizedGRBMTrainer::GeneralizedGRBMTrainer()
{
}
...
GeneralizedGRBMTrainer::GeneralizedGRBMTrainer(GeneralizedGRBM & rbm) {
    initMomentum(rbm);
    initGradient(rbm);
    initDataMean(rbm);
    initRBMExpected(rbm);
...
void GeneralizedGRBMTrainer::initMomentum(GeneralizedGRBM & rbm) {
...
void GeneralizedGRBMTrainer::initMomentum() {
...
void GeneralizedGRBMTrainer::initGradient(GeneralizedGRBM & rbm) {
...
void GeneralizedGRBMTrainer::initGradient() {
...
void GeneralizedGRBMTrainer::initDataMean(GeneralizedGRBM & rbm) {
...
void GeneralizedGRBMTrainer::initRBMExpected() {
    sampleMean.visible.setConstant(0.0);
    sampleMean.visible2.setConstant(0.0);  // Gaussian Unit
    sampleMean.hidden.setConstant(0.0);
}

void GeneralizedGRBMTrainer::train(GeneralizedGRBM & rbm, std::vector<std::vector<double>> & dataset) {
...
// 1回だけ学習
void GeneralizedGRBMTrainer::trainOnce(GeneralizedGRBM & rbm, std::vector<std::vector<double>> & dataset) {
...
void GeneralizedGRBMTrainer::calcDataMean(GeneralizedGRBM & rbm, std::vector<std::vector<double>> & dataset, std::vector<int> & data_indexes) {
    // 0埋め初期化
    initDataMean();
...
void GeneralizedGRBMTrainer::calcRBMExpectedCD(GeneralizedGRBM & rbm, std::vector<std::vector<double>> & dataset, std::vector<int> & data_indexes) {
    // 0埋め初期化
    initRBMExpected();
    ...
    for (auto & n : data_indexes) {
        ...
        // CD-K
        GeneralizedGRBMSampler sampler;
        for (int k = 0; k < cdk; k++) {
...
// 勾配の計算
void GeneralizedGRBMTrainer::calcGradient(GeneralizedGRBM & rbm, std::vector<int> & data_indexes) {
    // 勾配ベクトルリセット
    initGradient();
...
void GeneralizedGRBMTrainer::updateMomentum(GeneralizedGRBM & rbm) {
    for (int i = 0; i < rbm.getVisibleSize(); i++) {
...
// パラメータの更新
void GeneralizedGRBMTrainer::updateParams(GeneralizedGRBM & rbm) {
    for (int i = 0; i < rbm.getVisibleSize(); i++) {
