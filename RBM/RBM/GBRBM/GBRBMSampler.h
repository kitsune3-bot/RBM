...
double Sampler<GBRBM>::gibbsSamplingVisible(GBRBM &rbm, int vindex) {
...
double Sampler<GBRBM>::gibbsSamplingHidden(GBRBM &rbm, int hindex) {
	std::random_device rd;
	std::mt19937 mt(rd());
...
Eigen::VectorXd & Sampler<GBRBM>::blockedGibbsSamplingVisible(GBRBM &rbm) {
...
Eigen::VectorXd & Sampler<GBRBM>::blockedGibbsSamplingHidden(GBRBM &rbm) {
...
double Sampler<GBRBM>::updateByGibbsSamplingVisible(GBRBM &rbm, int vindex) {
	std::random_device rd;
	std::mt19937 mt(rd());
	std::normal_distribution<double> dist(rbm.meanVisible(vindex), sqrt(1 / rbm.params.lambda(vindex)));
...
double Sampler<GBRBM>::updateByGibbsSamplingHidden(GBRBM &rbm, int hindex) {
	std::random_device rd;
	std::mt19937 mt(rd());
	std::uniform_real_distribution<double> dist(0.0, 1.0);
...
Eigen::VectorXd & Sampler<GBRBM>::updateByBlockedGibbsSamplingVisible(GBRBM &rbm) {
...
Eigen::VectorXd & Sampler<GBRBM>::updateByBlockedGibbsSamplingHidden(GBRBM &rbm) {
	for (int j = 0; j < rbm.getHiddenSize(); j++) {
