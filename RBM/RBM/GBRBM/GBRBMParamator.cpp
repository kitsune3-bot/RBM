...
// 可視変数の総数を返す
size_t GBRBMParamator::getVisibleSize() {
	return vSize;
}

// 隠れ変数の総数を返す
size_t GBRBMParamator::getHiddenSize() {
	return hSize;
}

// 可視変数のバイアスを返す
double GBRBMParamator::getVisibleBias(int vindex) {
	return b(vindex);
}

// 可視変数のバイアスベクトルを返す
Eigen::VectorXd GBRBMParamator::getVisibleBiasVector() {
	return b;
}

// 可視変数の逆分散を返す
double GBRBMParamator::getVisibleLambda(int vindex) {
	return lambda(vindex);
}

// 可視変数の逆分散ベクトルを返す
Eigen::VectorXd GBRBMParamator::getVisibleLambdaVector() {
	return lambda;
}

// 隠れ変数のバイアスを返す
double GBRBMParamator::getHiddenBias(int hindex) {
	return c(hindex);
}

// 隠れ変数のバイアスベクトルを返す
Eigen::VectorXd GBRBMParamator::getHiddenBiasVector() {
	return c;
}

// ウェイトパラメータを返す
double GBRBMParamator::getWeight(int vindex, int hindex) {
	return w(vindex, hindex);
}

// ウェイト行列を返す
Eigen::MatrixXd GBRBMParamator::getWeightMatrix() {
	return w;
}
