...
// パラメータ情報のシリアライズ
inline std::string GBRBMParamator::serialize() {
	nlohmann::json json;
	json["vSize"] = vSize;
	json["hSize"] = hSize;
...
// パラメータ情報のデシリアライズ
inline void GBRBMParamator::deserialize(std::string js) {
...
inline GBRBMParamator::GBRBMParamator(size_t v_size, size_t h_size) {
	vSize = v_size;
	hSize = h_size;
...
inline void GBRBMParamator::initParams() {
	b.resize(vSize);
	b.setConstant(0.0);
	lambda.resize(vSize);
...
inline void GBRBMParamator::initParamsRandom(double range_min, double range_max) {
	b.resize(vSize);
	c.resize(hSize);
	w.resize(vSize, hSize);
