...
double GeneralizedRBM::expectedValueVis(int vindex) {
	...
	for (int c = 0; c < max_count; c++, sc++) {
		...
		// 項計算
		double term = exp(nodes.getVisibleLayer().dot(params.b)) * this->nodes.v(vindex);
		for (int j = 0; j < hSize; j++) {
			auto mu_j = mu(j);
...
double GeneralizedRBM::expectedValueHid(int hindex) {
	...
	for (int c = 0; c < max_count; c++, sc++) {
		...
		// 項計算
		// TODO: 総和計算を離散型と連続型で分けよ

		double term = exp(nodes.getVisibleLayer().dot(params.b));

		auto sum_exp_j = [&](int j) {
			double sum = 0.0;
			auto mu_j = mu(j);
		...
		auto integral_exp_j = [&](int j) { 
			auto mu_j = this->mu(j);
			double value = (( hMax * exp(hMax * mu_j) - hMin * exp(hMin * mu_j)) / mu_j) - ( (exp(hMax * mu_j) - exp(hMin * mu_j)) / (mu_j * mu_j));

			return value;
		};


		term *= exp(nodes.getVisibleLayer().dot(params.b));
		...
		for (int l = 0; l < hSize; l++) {
			...
			auto sum_h_l = realFlag ? sum_h_l_real(mu_l) : sum_h_l_discrete(mu_l);
