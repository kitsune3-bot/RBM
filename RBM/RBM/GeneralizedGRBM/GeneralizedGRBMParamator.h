...
inline GeneralizedGRBMParamator::GeneralizedGRBMParamator(size_t v_size, size_t h_size) {
	vSize = v_size;
	hSize = h_size;
...
inline void GeneralizedGRBMParamator::initParams() {
	b.resize(vSize);
	b.setConstant(0.0);
	lambda.resize(vSize);
...
inline void GeneralizedGRBMParamator::initParamsRandom(double range_min, double range_max) {
	b.resize(vSize);
	c.resize(hSize);
	w.resize(vSize, hSize);
